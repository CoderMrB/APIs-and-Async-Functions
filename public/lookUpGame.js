...
async function retrieveGameData(page_number){
    ...
    let gameData = await fetch (`https://api.boardgameatlas.com/api/search?min_players=${parseInt(searchPlayersValue)}&max-playtime<=${searchTimeValue}&limit=100&client_id=DwheEZXOaI`)
    let gameDataResult = await gameData.json()
...
async function get_all_data() {
    const total_results_response = await fetch(`https://api.boardgameatlas.com/api/search?min_players=${parseInt(searchPlayersValue)}&min-playtime<=${searchTimeValue}&limit=100&client_id=DwheEZXOaI`);
    ...
    gameNameDisplay.innerHTML = `${chosenGame.name} can be played with ${chosenGame.min_players} people and shouldn't take longer than ${chosenGame.max_playtime} minutes to complete` 
